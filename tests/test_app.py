...
from dotserve import AdminDash, constants
from dotserve.app import (
    App,
    ComponentCallable,
    DefaultState,
...
from dotserve.components import Box, Component, Cond, Fragment, Text
from dotserve.event import Event, get_hydrate_event
from dotserve.middleware import HydrateMiddleware
from dotserve.model import Model
from dotserve.state import State, StateManagerRedis, StateUpdate
from dotserve.style import Style
from dotserve.utils import format
from dotserve.vars import ComputedVar
...
def test_multiple_states_error(monkeypatch, test_state, redundant_test_state):
    """Test that an error is thrown when multiple classes subclass ds.State.

    Args:
        monkeypatch: Pytest monkeypatch object.
        test_state: A test state subclassing ds.State.
        redundant_test_state: Another test state subclassing ds.State.
    """
...
async def test_upload_file_without_annotation(state, tmp_path, token):
    """Test that an error is thrown when there's no param annotated with ds.UploadFile or List[UploadFile].
    ...
    assert (
        err.value.args[0]
        == f"`{state.get_name()}.handle_upload2` handler should have a parameter annotated as List[ds.UploadFile]"
    )
