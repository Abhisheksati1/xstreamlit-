...
from nextpy.utils.hosting import DeploymentPrepInfo
...
def test_login_success_existing_token(mocker):
    mocker.patch(
        "nextpy.utils.hosting.authenticated_token",
...
def test_login_success_on_browser(mocker):
    ...
    mock_authenticate_on_browser = mocker.patch(
        "nextpy.utils.hosting.authenticate_on_browser", return_value="fake-token"
    )
    result = runner.invoke(cli, ["login"])
    assert result.exit_code == 0
...
def test_login_fail(mocker):
    ...
    mocker.patch(
        "nextpy.utils.hosting.get_existing_access_token", return_value=("", "")
    )
    mocker.patch("nextpy.utils.hosting.authenticate_on_browser", return_value="")
    result = runner.invoke(cli, ["login"])
...
def test_deploy_non_interactive_success(
    ...
    mock_deploy = mocker.patch(
        "nextpy.utils.hosting.deploy",
        return_value=Mock(
            frontend_url="fake-frontend-url", backend_url="fake-backend-url"
        ),
    )
    mocker.patch("nextpy.utils.hosting.wait_for_server_to_pick_up_request")
    mocker.patch("nextpy.utils.hosting.display_deploy_milestones")
    mocker.patch("nextpy.utils.hosting.poll_backend", return_value=True)
    mocker.patch("nextpy.utils.hosting.poll_frontend", return_value=True)
...
def test_deploy_interactive(
    ...
    mocker.patch(
        "nextpy.utils.hosting.check_requirements_for_non_nextpy_packages",
        return_value=True,
    )
    mocker.patch(
        "nextpy.utils.hosting.prepare_deploy",
    ...
    mocker.patch(
        "nextpy.utils.hosting.interactive_get_deployment_key_from_user_input",
    ...
    mocker.patch(
        "nextpy.utils.hosting.interactive_prompt_for_envs", return_value=user_input_envs
    )
    fake_export_dir = "fake-export-dir"
    mocker.patch("tempfile.mkdtemp", return_value=fake_export_dir)
    mocker.patch("nextpy.cli.export")
