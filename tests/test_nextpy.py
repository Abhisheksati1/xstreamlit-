...
def test_login_success(mocker):
    ...
    mock_validate_token = mocker.patch(
        "nextpy.utils.hosting.validate_token_with_retries"
    )
...
def test_login_existing_token_but_invalid(mocker):
    ...
    mocker.patch(
        "nextpy.utils.hosting.validate_token",
        side_effect=ValueError("token not valid"),
    )
    mock_delete_token_from_config = mocker.patch(
        "nextpy.utils.hosting.delete_token_from_config"
    )
    result = runner.invoke(cli, ["login"])
...
def test_login_no_existing_token_fetched_valid(mocker):
    ...
    mocker.patch(
        "nextpy.utils.hosting.authenticate_on_browser",
    ...
    mock_save_token_to_config = mocker.patch(
        "nextpy.utils.hosting.save_token_to_config"
    )
    result = runner.invoke(cli, ["login"])
    assert result.exit_code == 0
...
def test_login_no_existing_token_fetch_none(mocker):
    ...
    mocker.patch(
        "nextpy.utils.hosting.authenticate_on_browser", return_value=(None, None)
    )
    result = runner.invoke(cli, ["login"])
    assert result.exit_code == 1
